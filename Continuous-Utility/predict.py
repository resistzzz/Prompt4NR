...
def ddp_main(rank, world_size, args):
    ...
    # load tokenizer
    tokenizer, conti_tokens1, conti_tokens2, conti_tokens3 = load_tokenizer(args.model_name, args)
    conti_tokens = [conti_tokens1, conti_tokens2, conti_tokens3]
    ...
    test_dataset = MyDataset(args, tokenizer, news_dict, conti_tokens, status='test')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='../PLMs/bert-base-uncased', type=str)

    parser.add_argument('--ratio', default=0.5, type=float, help='ratio of all datasets')
    ...
    parser.add_argument('--num_conti3', default=3, type=int, help='number of continuous tokens')
    ...
    if args.ratio == 1.0:
        if args.data_path == '../DATA/MIND-Demo':
            if args.log:
                ...
                log_file = './log-Test/' + 'Tbs' + str(args.test_batch_size) + \
                           '-n' + str(args.num_conti1) + str(args.num_conti2) + str(args.num_conti3) + \
            ...
            if args.log:
                ...
                log_file = './log-Test-Small/' + 'Tbs' + str(args.test_batch_size) + \
                           '-n' + str(args.num_conti1) + str(args.num_conti2) + str(args.num_conti3) + \
                           '-' + str(datetime.now())[-5:]+'.txt'
                args.log_file = log_file
    else:
        if args.data_path == '../DATA/MIND-Demo':
            if args.log:
                if not os.path.exists('./log-Test-Few'):
                    os.makedirs('./log-Test-Few')
                log_file = './log-Test-Few/' + 'Tbs' + str(args.test_batch_size) + \
                           '-ratio' + str(args.ratio) + \
                           '-' + str(datetime.now())[-5:]+'.txt'
                args.log_file = log_file
        else:
            if args.log:
                if not os.path.exists('./log-Test-Small-Few'):
                    os.makedirs('./log-Test-Small-Few')
                log_file = './log-Test-Small-Few/' + 'Tbs' + str(args.test_batch_size) + \
                           '-ratio' + str(args.ratio) + \
                           '-' + str(datetime.now())[-5:]+'.txt'
                args.log_file = log_file
